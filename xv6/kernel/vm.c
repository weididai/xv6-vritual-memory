...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = PGSIZE; i < sz; i += PGSIZE){
	...
	//child has parent's page protection bit
...
int munprotectHelper(void *addr, int len) {
	pde_t *pgdir = proc->pgdir;
	uint address = (uint)addr;
	for(int i = 0; i < len; i++) {
                pde_t *pte;
                if(address > proc->sz-1 || address < PGSIZE) 
                        return -1;
                if((pte = walkpgdir(pgdir, (void *)address, 0)) == 0) 
                        return -1;
                address += PGSIZE;
        }
...
int mprotectHelper(void *addr, int len){		
	pde_t *pgdir = proc->pgdir;
	uint address = (uint)addr;
	for(int i = 0; i < len; i++) {
		pde_t *pte;
